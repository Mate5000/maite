...
document.getElementById("button").onclick = () => {
    ...
    if(tema != '' && betoltes2.style.display == "none"){
        ...
        const hossz = document.getElementById('karakterszam').value;
        const tudasszint = document.getElementById('tudasszint').value;
        const egyeb = document.getElementById('egyeb').value;
        const alkalmazas = document.getElementById("app_id").value;

        const xhr = new XMLHttpRequest();
        const url = '../../api/request.php';
        const params = `app=${alkalmazas}&tema=${tema}&hossz=${hossz}&tudasszint=${tudasszint}&egyeb=${egyeb}`;
        ...
        alert("Nincs megadva minden információ, vagy már fut egy generálás.")
    } else {
