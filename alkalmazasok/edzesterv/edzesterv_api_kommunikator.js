...
document.querySelector('button').onclick = () => {
    ...
    if(betoltes2.style.display == "none"){
        ...
        const suly = document.getElementById('suly').value;
        const magassag = document.getElementById('magassag').value;
        const hetihany = document.getElementById('hetihany').value;
        const napimennyiseg = document.getElementById('napimennyiseg').value;
        const milyentipus = document.getElementById('milyentipus').value;
        const elerni = document.getElementById('elerni').value;
        const eszkozok = document.getElementById("eszkozok").value;
        const egyeb = document.getElementById("egyeb").value;

        const xhr = new XMLHttpRequest();
        const url = '../../api/request.php';
        const params = `app=${alkalmazas}&k=${kor}&s=${suly}&m=${magassag}&h=${hetihany}&hpn=${napimennyiseg}&et=${milyentipus}&c=${elerni}&e=${eszkozok}&egyeb=${egyeb}`;
