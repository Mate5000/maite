...
document.querySelector('button').onclick = () => {
    ...
    if(betoltes2.style.display == "none"){
        ...
        const tananyag = document.getElementById('tananyag').value;
        const tudasszint = document.getElementById('tudasszint').value;
        const egyeb = document.getElementById('parameterek').value;

        const xhr = new XMLHttpRequest();
        const url = '../../api/request.php';
        const params = `app=${alkalmazas}&tantargy=${tantargy}&tananyag=${tananyag}&tudasszint=${tudasszint}&egyeb=${egyeb}`;
